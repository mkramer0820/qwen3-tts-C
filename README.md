# Qwen3-TTS C - AMD ROCm Development Fork

This fork adds an AMD ROCm path to the existing Qwen3-TTS pure-C engine. Its
primary acceptance target is the **AMD Radeon AI PRO R9700** (RDNA4, `gfx1201`,
32 GiB) on Windows 11 through Ubuntu 24.04 WSL2 and Docker. RDNA3/RDNA4 are the
intended AMD scope; AMD Instinct/CDNA/MI-series hardware is not a project target.
...
- Strict backend detection and a GPU self-test that cannot report a CPU fallback as
  a ROCm pass.
- Per-operation CPU recomputation when a HIP allocation, copy, or GEMM fails.
- ROCm-aware expressivity LoRA training checks, BF16 validation, label-alignment
  fixes, and dependency documentation.
- A guided Windows/WSL2 container path pinned to ROCm 7.2.1, PyTorch 2.9.1,
...
```powershell
# First run: install/check Ubuntu WSL, ROCm/ROCDXG, Docker, and the image
powershell -ExecutionPolicy Bypass -File .\rocm-wsl.ps1

# After any requested restart and Ubuntu first-login setup
.\rocm-wsl.ps1 -Action doctor
.\rocm-wsl.ps1 -Action build
.\rocm-wsl.ps1 -Action shell
```

The first command may stop after installing Ubuntu. Open Ubuntu once, create the
Linux username/password, then rerun it. The `build` action must pass both PyTorch
BF16 validation and the C `--gpu-selftest` before this fork can claim basic R9700
operation.

## ROCm Validation Matrix

| Area | Current evidence | R9700 result |
|---|---|---|
| PowerShell and Compose configuration | Static parse/schema checks pass | Pending |
| ROCm Docker image build | Configuration written | Not run |
| HIP C compilation (`gfx1201`) | Source review only | Not run |
| C GPU self-test and CPU fallback | Source review only | Not run |
| PyTorch BF16 forward/backward | Guard implemented | Not run |
| LoRA train/export | Code and shape checks implemented | Not run |
| Voice clone/name/load | Existing workflow reviewed | Not run |
| `.expr`, emotion, and audible output | Restrictions documented | Not run |
...
## Inherited Audio Samples

These upstream samples were generated with the 0.6B model on CPU. They were not
generated by the ROCm branch and do not demonstrate R9700 behavior or quality.
...
## CPU and Existing Backend Quick Start

This is the inherited native CPU path. Use the R9700 quick start above for ROCm.

```bash
# Clone and build
git clone git@github.com:mkramer0820/qwen3-tts-C.git
cd qwen3-tts-C
make blas
...
## Inherited Engine Features

The capabilities below come from the existing engine. ROCm compatibility is
...
> **Benchmark provenance:** every number in this section is inherited from the
> pre-ROCm project documentation. No table below contains an AMD Radeon AI PRO
> R9700 measurement. R9700 results must not be added until the documented doctor,
> build, self-test, generation, and training acceptance steps have been run.

### AMD Radeon AI PRO R9700 Performance Status

`Undetermined` is intentional: these cells are the measurements required from the
target machine, not estimates copied from a different GPU.
...
| 1.7B cloned `.qvoice` | ROCm BF16 + `--icl-only` | Undetermined | Undetermined | Undetermined | Not run |
| 1.7B expression output | ROCm BF16 + `.expr` + `--emotion` | Undetermined | Undetermined | Undetermined | Not run |
| HTTP serving | ROCm BF16, 1/4/8 clients | Undetermined | Undetermined | Undetermined | Not run |
| LoRA training | PyTorch ROCm BF16 | Undetermined steps/s | N/A | Undetermined | Not run |

| Backend | GPU-resident coverage | Quantized acceleration | Published numbers in this fork |
|---|---|---|---|
| CPU | Inherited native implementation | INT8/INT4 | Inherited upstream results below |
| Apple Metal | Inherited fused resident path | Backend-specific | Inherited upstream results below |
| NVIDIA CUDA | Inherited fused resident path | CUDA DP4A/quant-mixed options | Inherited upstream results below |
| AMD ROCm | BF16 matvec/matmat only; remaining operations use CPU | INT8/INT4 kernels remain on CPU | **Undetermined** |

The first R9700 report must record model size, precision, command line, seed,
generated audio duration, wall time, RTF, time-to-first-audio, peak VRAM, driver,
ROCm/PyTorch versions, and whether any CPU fallback warning occurred.
...
per matrix operation; other kernels retain the CPU fallback. **No ROCm performance
numbers exist yet.** See [AMD ROCm](docs/amd-rocm.md).
